...
def _proxy_from_conf(proxies_conf):
    """从配置 dict 提取单个代理 URL（兼容 httpx 0.28+）"""
    if not proxies_conf:
        return None
    return proxies_conf.get("https") or proxies_conf.get("http") or None
...
class TokenManager:
    ...
    proxy = _proxy_from_conf(proxies_conf)
    ...
    def gen_real_msToken(cls) -> str:
        ...
        with httpx.Client(transport=transport, proxy=cls.proxy) as client:
            try:
            ...
            #                              .format(cls.token_conf["url"], cls.proxy, cls.__name__, exc)
            #                              )
            #
    ...
    def gen_ttwid(cls, cookie: str) -> str:
        ...
        with httpx.Client(transport=transport, proxy=cls.proxy) as client:
            try:
                ...
                # 捕获所有与 httpx 请求相关的异常情况 (Captures all httpx request-related exceptions)
                raise APIConnectionError("请求端点失败，请检查当前网络环境。 链接：{0}，代理：{1}，异常类名：{2}，异常详细信息：{3}"
                                         .format(cls.ttwid_conf["url"], cls.proxy, cls.__name__, exc)
                                         )
    ...
    def gen_odin_tt(cls):
        ...
        with httpx.Client(transport=transport, proxy=cls.proxy) as client:
            try:
                ...
                # 捕获所有与 httpx 请求相关的异常情况 (Captures all httpx request-related exceptions)
                raise APIConnectionError("请求端点失败，请检查当前网络环境。 链接：{0}，代理：{1}，异常类名：{2}，异常详细信息：{3}"
                                         .format(cls.odin_tt_conf["url"], cls.proxy, cls.__name__, exc)
                                         )
...
class SecUserIdFetcher:
    ...
    async def get_secuid(cls, url: str) -> str:
        ...
        async with httpx.AsyncClient(
                transport=transport, proxy=TokenManager.proxy, timeout=10
        ) as client:
            try:
                ...
                # 捕获所有与 httpx 请求相关的异常情况 (Captures all httpx request-related exceptions)
                raise APIConnectionError("请求端点失败，请检查当前网络环境。 链接：{0}，代理：{1}，异常类名：{2}，异常详细信息：{3}"
                                         .format(url, TokenManager.proxy, cls.__name__, exc)
                                         )
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                raise APIConnectionError("连接端点失败，检查网络环境或代理：{0} 代理：{1} 类名：{2}"
                                         .format(url, TokenManager.proxy, cls.__name__),
                                         )
